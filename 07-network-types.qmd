...
We start by loading the necessary packages and reading an edgelist from a `.csv` file into a data frame:
...
We can construct directed and undirected variants in the following way:

```{r}

directed <- network(edgelist, directed = TRUE)

undirected <- network(edgelist, directed = FALSE)
```
...
We can use the bipartite nature of this network for plotting by passing the `gmode = "twomode"` flag to the `gplot(...)` function:
